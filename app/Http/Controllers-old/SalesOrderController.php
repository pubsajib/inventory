...
class SalesOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($data['salesData'] as $item) {
            ...
            foreach($raw_all_order_no as $item1){
            	$all_order_no[] = $item1->order_no;
            }
            ...
            $total_item_price_returned = (isset($raw_total_returned[0]->total_item_price_returned) && !empty($raw_total_returned[0]->total_item_price_returned))? $raw_total_returned[0]->total_item_price_returned : 0;
            $item->ordered_quantity = $item->ordered_quantity - $item_total_returned;
            $item->invoiced_quantity = $item->invoiced_quantity - $item_total_returned;
            $item->order_amount = $item->order_amount - $total_item_price_returned;
        }
        /////123456///////////
    ...
    **/
    ...
    public function viewOrderDetails($orderNo){
       ...
       // d($data['saleData'],1);
        ...
        /////123456///////////
        $reference = $data['saleData']->reference;
        $raw_all_order_no = DB::table('sales_orders')->select('order_no')->where('order_reference',$reference)->get();
        $all_order_no = [];
        foreach($raw_all_order_no as $item){
        	$all_order_no[] = $item->order_no;
        }
        ...
        $item_total_returned = (isset($raw_total_returned[0]->item_quantity_returned) && !empty($raw_total_returned[0]->item_quantity_returned))? $raw_total_returned[0]->item_quantity_returned : 0;
        $total_item_price_returned = (isset($raw_total_returned[0]->total_item_price_returned) && !empty($raw_total_returned[0]->total_item_price_returned))? $raw_total_returned[0]->total_item_price_returned : 0;
        $data['invoiceData'][0]['quantity'] = $data['invoiceData'][0]['quantity'] - $item_total_returned;
        $data['orderQty'] = $data['orderQty'] - $item_total_returned;
        $data['saleData']->total = $data['saleData']->total - $total_item_price_returned;
        if(isset($data['shipmentList'][0]->total)){
            $data['shipmentList'][0]->total = $data['shipmentList'][0]->total - $item_total_returned;
        }
        /////123456///////////
//print "<pre>";print_r($data);print_r($raw_all_order_no);print_r($rall_order_no);exit;
