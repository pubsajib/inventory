...
class ReportController extends Controller
{
    ...
    */
    ...
    public function customerRanking(){
        $data['menu']     = 'report';
        $data['sub_menu'] = 'customer-ranking';

        $data['customers'] = $this->order->getSalseOrderByCustomer();

        foreach($data['customers'] as $key=>$val){
            $purchases = $this->order->getItemPurchaseByCustomer($val->debtor_no);
            $total_purchase = 0;
            foreach($purchases as $purchase){
                $purchase = $purchase->unit_price*$purchase->qty_invoiced;
                $total_purchase = $total_purchase+$purchase;
            }
            $data['customers'][$key]->total_purchase = $total_purchase;
        }

        return view('admin.report.customer_ranking', $data);
        //echo "<pre>"; print_r($data['customers']); echo "<pre>";
    }

    public function productRanking(){
        $data['menu']     = 'report';
        $data['sub_menu'] = 'product-ranking';

        $data['products'] = $this->order->getSalseOrderByProduct();

        foreach($data['products'] as $key=>$val){
            $purchases = $this->order->getItemPurchaseByStock($val->stock_id);
            $total_purchase = 0;
            foreach($purchases as $purchase){
                $purchase = $purchase->unit_price*$purchase->qty_invoiced;
                $total_purchase = $total_purchase+$purchase;
            }
            $data['products'][$key]->total_purchase = $total_purchase;
        }

        return view('admin.report.product_ranking', $data);
        //echo "<pre>"; print_r($data['products']); echo "<pre>";
    }

    public function customerLedger(){
        $data['menu']     = 'report';
        $data['sub_menu'] = 'customer-ledger';
        $data['bf_sales'] = array();
        $data['sales'] = array();
        ...
        $data['customers'] = DB::table('debtors_master')->orderBy('debtor_no', 'desc')->get();
        return view('admin.report.customer_ledger', $data);
    }

    public function searchCustomerLedger(Request $request){
        $data['menu']     = 'report';
        $data['sub_menu'] = 'customer-ledger';
        $data['companyData'] = DB::table('preference')->where('category', 'company')->get();
        $data['customer_details'] = DB::table('debtors_master')->where('debtor_no', $request->customer_id)->first();
        $data['customers'] = DB::table('debtors_master')->orderBy('debtor_no', 'desc')->get();
        ...
        /*Calculating B/F*/
        $data['bf_sales'] = DB::table('sales_orders')
            ->where('debtor_no',$request->customer_id)
            ->where('trans_type',202)
            ->where('created_at','<=',$start_date)
            ->get();
        foreach($data['bf_sales'] as $key=>$value){
            $items = DB::table('sales_order_details')->where('order_no',$value->order_no)->get();
            $returned_item = DB::table('returned_items')->where('order_no',$value->order_no)->first();
            if(empty($returned_items)){
                $data['bf_sales'][$key]->return_item_price = 0;
            }
            else{
                $data['bf_sales'][$key]->return_item_price = $returned_item->total_item_price_returned;
            }
            $data['bf_sales'][$key]->items = $items;
        }
        /*Calculating B/F ends*/


        $data['sales'] = DB::table('sales_orders')
            ->where('debtor_no',$request->customer_id)
            ->where('trans_type',202)
            ->where('created_at','>=',$start_date)
        ...
        foreach($data['sales'] as $key=>$value){
            ...
            $returned_item = DB::table('returned_items')->where('order_no',$value->order_no)->first();
            if(empty($returned_items)){
                $data['sales'][$key]->return_item_price = 0;
            }
            else{
                $data['sales'][$key]->return_item_price = $returned_item->total_item_price_returned;
            }
            $data['sales'][$key]->items = $items;
        }
        return view('admin.report.customer_ledger', $data);
    }
