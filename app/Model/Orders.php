...
//echo $where;
    ...
    public function getSalseOrderByCustomer()
    {
        $data = DB::table('sales_orders')
            ->leftJoin('debtors_master', 'sales_orders.debtor_no', '=', 'debtors_master.debtor_no')
            ->select('sales_orders.*', 'debtors_master.*', DB::raw('SUM(sales_orders.total) as total_sales'), DB::raw('SUM(sales_orders.paid_amount) as total_paid_amount'))
            ->groupBy('sales_orders.debtor_no')
            ->orderBy('total_sales','desc')
            ->get();
        return $data;
    }

    public function getItemPurchaseByCustomer($debtor_no)
    {
        $data = DB::table('sales_orders')
            ->join('sales_order_details', 'sales_orders.order_no', '=', 'sales_order_details.order_no')
            ->join('item_code', 'sales_order_details.stock_id', '=', 'item_code.stock_id')
            ->join('purch_order_details', 'item_code.id', '=', 'purch_order_details.item_code')
            ->select(DB::raw('SUM(purch_order_details.qty_invoiced) as qty_invoiced'), DB::raw('SUM(purch_order_details.unit_price) as unit_price'))
            ->groupBy('purch_order_details.item_code')
            ->where('sales_orders.debtor_no',$debtor_no)
            ->get();
        return $data;
    }

    public function getSalseOrderByProduct()
    {
        $data = DB::table('sales_orders')
            ->leftJoin('sales_order_details', 'sales_orders.order_no', '=', 'sales_order_details.order_no')
            ->select('sales_order_details.*', DB::raw('SUM(sales_order_details.unit_price * sales_order_details.quantity) as total_sales'))
            ->groupBy('sales_order_details.stock_id')
            ->orderBy('total_sales','desc')
            ->get();
        return $data;
    }

    public function getItemPurchaseByStock($stock_id)
    {
        $data = DB::table('sales_orders')
            ->join('sales_order_details', 'sales_orders.order_no', '=', 'sales_order_details.order_no')
            ->join('item_code', 'sales_order_details.stock_id', '=', 'item_code.stock_id')
            ...
            ->select(DB::raw('SUM(purch_order_details.qty_invoiced) as qty_invoiced'), DB::raw('SUM(purch_order_details.unit_price) as unit_price'))
            ->groupBy('purch_order_details.item_code')
            ->where('item_code.stock_id',$stock_id)
            ->get();
        return $data;
    }
