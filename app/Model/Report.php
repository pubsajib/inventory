...
$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale,infos.stock_id FROM sales_orders
    ...
    public function invoicesListOnDate($start_date, $end_date){
    	$data = DB::table('sales_orders')
    			->leftjoin("debtors_master",'debtors_master.debtor_no','=','sales_orders.debtor_no')
    			->leftjoin("returned_items",'sales_orders.order_no','=','returned_items.order_no')
    			->where('sales_orders.trans_type',SALESINVOICE)
    			->where('sales_orders.ord_date','>=',$start_date)
    			->where('sales_orders.ord_date','<=',$end_date)
    			->orderBy('sales_orders.ord_date','desc')
    			->select('sales_orders.order_reference_id as order_no','sales_orders.order_no as invoice_no','sales_orders.order_reference','sales_orders.reference','debtors_master.name','sales_orders.total','sales_orders.ord_date','returned_items.total_item_price_returned')
    			->take(5)
    			->get();

    	return $data;
    }
    public function highestInvoicesList(){
    	$data = DB::table('sales_orders')
    			->leftjoin("debtors_master",'debtors_master.debtor_no','=','sales_orders.debtor_no')
    			->where('sales_orders.trans_type',SALESINVOICE)
    			->orderBy('sales_orders.total','desc')
    			->select('sales_orders.order_reference_id as order_no','sales_orders.order_no as invoice_no','sales_orders.order_reference','sales_orders.reference','debtors_master.name','sales_orders.total','sales_orders.ord_date')
    			->take(5)
    			->get();
    ...
    public function lowestInvoicesList(){
    	$data = DB::table('sales_orders')
    			->leftjoin("debtors_master",'debtors_master.debtor_no','=','sales_orders.debtor_no')
    			->where('sales_orders.trans_type',SALESINVOICE)
    			->orderBy('sales_orders.total','asc')
    			->select('sales_orders.order_reference_id as order_no','sales_orders.order_no as invoice_no','sales_orders.order_reference','sales_orders.reference','debtors_master.name','sales_orders.total','sales_orders.ord_date')
    			->take(5)
    			->get();
