  	...
  	if( $type=='daily' ){
  		 ...
  		 if($item == 'all' && $customer == 'all' && $location == 'all'){
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale FROM sales_orders
				...
				"));  		 	
  		 }else if($item == 'all' && $customer == 'all' && $location != 'all'){
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale FROM sales_orders
				LEFT JOIN(SELECT sale_purch_detail.order_no, count(sale_purch_detail.order_no)as total_order,SUM(sale_purch_detail.quantity)as qty,SUM(sale_purch_detail.sale) as sale,SUM(sale_purch_detail.purchase)as purchase FROM(SELECT sales.*,(purchase.purchase_rate*sales.quantity)as purchase FROM(SELECT sods.order_no,sods.stock_id,sods.quantity,(sods.discount_price-(sods.discount_price*item_tax_types.tax_rate/100)) as sale FROM(SELECT sod.`id`, sod.`order_no`, sod.`stock_id`, sod.`unit_price`*sod.`quantity`-(sod.`unit_price`*sod.`quantity`*`discount_percent`)/100 as discount_price ,sod.`discount_percent`,sod.`quantity`,sod.`unit_price`,sod.`tax_type_id` FROM `sales_order_details` as sod WHERE `trans_type`=202)sods
				LEFT JOIN item_tax_types
				ON item_tax_types.id = sods.tax_type_id)sales
				...
				ON infos.order_no = sales_orders.order_no
				WHERE sales_orders.trans_type = 202 
				AND sales_orders.from_stk_loc = '$location'
   			...
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale FROM sales_orders
				...
				ON infos.order_no = sales_orders.order_no
				WHERE sales_orders.trans_type = 202 
				AND sales_orders.debtor_no = '$customer'
   			...
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale FROM sales_orders
				...
				ON infos.order_no = sales_orders.order_no
				WHERE sales_orders.trans_type = 202 
				AND sales_orders.debtor_no = '$customer' AND sales_orders.from_stk_loc = '$location'
   			...
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale,infos.stock_id FROM sales_orders
				...
				GROUP BY sale_purch_detail.order_no)infos
				ON infos.order_no = sales_orders.order_no
				WHERE sales_orders.trans_type = 202 AND infos.stock_id = '$item' AND sales_orders.from_stk_loc = '$location'
				GROUP BY sales_orders.ord_date 
				ORDER BY sales_orders.ord_date DESC
				"));  		 	
  		 }else if($item !== 'all' && $customer != 'all' && $location == 'all'){
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale,infos.stock_id FROM sales_orders
				...
				GROUP BY sale_purch_detail.order_no)infos
				ON infos.order_no = sales_orders.order_no
				WHERE sales_orders.trans_type = 202 AND infos.stock_id = '$item' AND sales_orders.debtor_no = '$customer'
				GROUP BY sales_orders.ord_date 
				ORDER BY sales_orders.ord_date DESC
				"));  		 	
  		 }else if($item !== 'all' && $customer != 'all' && $location != 'all'){
   			$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale,infos.stock_id FROM sales_orders
				...
				GROUP BY sale_purch_detail.order_no)infos
				ON infos.order_no = sales_orders.order_no
				WHERE sales_orders.trans_type = 202 AND infos.stock_id = '$item' AND sales_orders.debtor_no = '$customer' AND sales_orders.from_stk_loc = '$location'
				GROUP BY sales_orders.ord_date 
				ORDER BY sales_orders.ord_date DESC
				"));  		 	
  		 }
